...
function run_ex2(; ceed_spec, dim, mesh_order, sol_order, num_qpts, prob_size)
    ...
    with_array_read(mesh_coords, MEM_HOST) do x_host
        with_array(u, MEM_HOST) do u_host
            for i=1:sol_size
                u_host[i] = sum(x_host[i .+ (0:dim-1)*sol_size])
            end
        end
    end
    ...
    sa = with_array_read(x -> sum(abs,x), v, MEM_HOST)
